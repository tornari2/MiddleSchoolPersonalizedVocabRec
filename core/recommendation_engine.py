...
from ..utils.reference_data_loader import ReferenceDataLoader
from ..services.openai_service import OpenAIService, RecommendationConfig
...
class RecommendationEngine:
    ...
    def generate_recommendations(self, student_id: str, student_profile: Dict[str, Any],
        ...
        try:
            ...
            recommendations = self._select_top_recommendations(scored_candidates, grade_level, student_profile)
    ...
    def _score_candidates(self, candidates: List[Dict[str, Any]], vocabulary_gaps: Dict[str, Any],
        ...
        for candidate in candidates:
            ...
            # Add personalization factor based on student ID to break ties
            # This ensures students with identical profiles get different rankings
            student_id = student_profile.get('student_id', '')
            if student_id:
                # Use student ID to create a consistent but unique factor for this student
                id_hash = hash(student_id)
                # Create a larger variation (±0.05) to break ties more effectively
                id_factor = ((id_hash % 100) - 50) / 1000  # Range: -0.05 to +0.05
                total_score += id_factor
    ...
    def _select_top_recommendations(self, scored_candidates: List[Dict[str, Any]],
                                  grade_level: int, student_profile: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Add personalization by rotating selection based on student ID
        student_id = student_profile.get('student_id', '')
        if student_id:
            # Use student ID to create a consistent rotation offset
            rotation_offset = hash(student_id) % len(scored_candidates)
            # Rotate the candidates list to create personalization
            scored_candidates = scored_candidates[rotation_offset:] + scored_candidates[:rotation_offset]

        # Ensure we have at least 10 recommendations by being more flexible
        selected = []
        seen_words = set()

        # First pass: Take top 8 highest-scoring candidates regardless of type
        for candidate in scored_candidates:
            if len(selected) >= 8:
                break
        ...
        # Second pass: Add 2 more candidates, prioritizing academic words if we have few
        academic_count = sum(1 for c in selected if c.get('word', '').lower() in self.academic_words)
        ...
        for candidate in scored_candidates:
            ...
            # If we have fewer than 2 academic words, prioritize adding academic words
            if academic_count < 2 and is_academic:
                selected.append(candidate)
                seen_words.add(word)
                academic_count += 1
            # Otherwise, just add the next highest scoring word
